...
export default function CategorySubscriptionPlan(props) {
	...
	let { plans, minHeight, minHeightKeyFeatures, maxWidthKeyFeatures, title, subtitle, planUnit } =
		props;
	...
	let currencySymbol = localStorage.getItem("user_currency")
		? JSON.parse(localStorage.getItem("user_currency"))[0].symbol
		: "$";
	let currencyMultiplier = localStorage.getItem("user_currency_multiplier")
		? parseFloat(localStorage.getItem("user_currency_multiplier"))
		: 1;
			...
			<div className="category-subscription-plans">
				<div className="container">
					...
					<p className="text-center subtitle">
						{subtitle
							? subtitle
							: "With one platform everyone sees the whole picture. All for one flat rate.. What's not to love about us."}
					</p>
							...
							<div className="plan-details">
								...
								<div className="price">
									...
									<button
										className="buy-btn"
										onClick={() => {
											SubscribeToService(MSelectedPlan);
										}}
									>
										{BuyBtnText}
									</button>
								...
								<div className="key-features">
									<ul style={{ minHeight: minHeightKeyFeatures || "auto" }}>
										{MSelectedPlan.key_features.map((feature, index) => {
											...
											if (highLightRegexCheck)
												feature.replace(
													highLightRegex,
													`<span class="highlight">${highLightRegexCheck[0]}</span>`
												);
													...
													{MSelectedPlan.features[f] ? (
														<>
															{typeof MSelectedPlan.features[f] === "boolean" ? (
																...
																<span className="f-count">{MSelectedPlan.features[f]}</span>
															)}
					...
					<div className="row justify-content-center desktop-only">
						<div className="col-12 col-sm-12 col-md-12 col-lg-11">
							<div className="custom-tab">
								<div className="custom-tab-header"></div>
													...
													<div className="item">
														<div className="sub-card fluid">
															...
															<div className="key-features">
																<ul
																	style={{
																		minHeight: minHeightKeyFeatures || "auto",
																		maxWidth: maxWidthKeyFeatures || "auto",
																	}}
																>
																	{plan.key_features.map((feature, index) => {
																		...
																		if (highLightRegexCheck)
																			feature = feature.replace(
																				highLightRegex,
																				`<span class="highlight">${highLightRegexCheck[1]}</span>`
																			);
															...
															<div className="sub-price">
																...
																<span className="amount">
																	{parseInt(
																		(currencyMultiplier * plan.price[planPeriod].amount) / 100
																	)}
																</span>
																<br />
															...
															<hr />
															<div className="subscription-points"></div>
															<div className="sub-price">
																<sup>
																	{/* {plan.price[planPeriod].sign} */}
																	{currencySymbol}
